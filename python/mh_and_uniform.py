...
if __name__ == '__main__':
    ...
    truep = []
    for i in range(50):
        res = go(net, 10000, t0, 2000, 60000)
        mh_res.append(res[1])
        uni.append(res[0])
...
fig, ax = plt.subplots()
ax.hist(diffs, bins = np.arange(-10, 10,.5))
ax.set_title('Histogram of Estimation Difference, Uni - MH')
fig.savefig('hist_of_diffs.png')
...
fig.savefig('against_no_attacker')
